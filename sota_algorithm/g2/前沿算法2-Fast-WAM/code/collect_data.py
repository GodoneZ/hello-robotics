"""Collect fresh 30 Hz G2 demonstrations for Fast-WAM fine-tuning.

The task expert comes from this chapter's local G2 runtime (shared with
Chapter 16).  The recorder stores measured qpos, commanded actions and three
synchronized cameras at 30 Hz; Fast-WAM's loader then samples 33-frame windows
(9 video frames + 32 action steps, action_video_freq_ratio=4).
"""
...
from environment import AutoExpert, COLORS, FastWAMSimulationConfig, G2Robot, G2Simulation
...
def collect_group(
    ...
    while accepted < target_successes:
        ...
        if len(recorder.qpos) < NUM_FRAMES:
            success = False
            reason = "too_short"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect fresh Fast-WAM G2 trajectories")
    parser.add_argument(
        "--clean-total", type=int, default=CLEAN_EPISODES,
        help="successful clean trajectories across selected colors",
    )
    parser.add_argument(
        "--randomized-total", type=int, default=RANDOMIZED_EPISODES,
        help="successful randomized trajectories across selected colors",
    )
    parser.add_argument("--randomized-noise", type=float, default=POSITION_NOISE)
    ...
    if args.randomized_noise < 0:
        parser.error("--randomized-noise must be non-negative")
