"""Pure NumPy joint validation for FastWAM G2 policy execution.

Fast-WAM and Motus share the same 16D absolute-qpos action contract, so the
joint-limit clipping and the grasp/release guard are reused verbatim from
Chapter 16's control.py.
"""
...
class GraspReleaseGuard:
    ...
    Fast-WAM predicts absolute qpos targets and has no force/contact controller.  In
    ...
    outside the FastWAM model so the official checkpoint and action format remain
    unchanged.
    """
    ...
    def _ee_position(robot) -> np.ndarray:
        ...
        ik = getattr(robot, "_fastwam_grasp_ik", None)
        if ik is None:
            ik = RightArmIK()
            robot._fastwam_grasp_ik = ik
