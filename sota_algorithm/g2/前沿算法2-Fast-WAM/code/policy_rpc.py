"""Small NumPy-over-TCP protocol that keeps Isaac Sim and Fast-WAM separate."""
...
class RemotePolicy:
    def __init__(self, host: str = "127.0.0.1", port: int = 8622, timeout: float = 900):
    ...
    def predict(self, images, state: np.ndarray, instruction: str) -> np.ndarray:
        ...
        if error:
            raise RuntimeError(f"remote Fast-WAM error: {error}")
