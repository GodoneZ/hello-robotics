...
class G2BaseController:
    ...
    def __init__(self, articulation, kinematics: SwerveKinematics, limits: ControlLimits) -> None:
        ...
        self.steering_indices = _find_joint_indices(dof_names, STEERING_JOINT_NAMES)
        self.drive_indices = _find_joint_indices(dof_names, DRIVE_JOINT_NAMES)
    ...
    def _apply_acceleration_limit(self, desired: ChassisVelocity, dt: float) -> ChassisVelocity:
    ...
    def _write_wheel_states(self, wheel_states: Sequence[WheelState]) -> None:
        ...
        drive_targets = np.array([state.wheel_speed for state in wheel_states], dtype=np.float64)
...
class PoseController:
    ...
    def compute(self, current: Pose2D, target: Pose2D) -> PoseControlResult:
        ...
        reached = distance_error <= self.position_tolerance and abs(yaw_error) <= self.yaw_tolerance
        if reached:
